...
namespace LogicUI.FancyTextRendering
{
    ...
    public static class Markdown
    {
        ...
        public static bool IsInTag(this string s,MarkdownRenderingSettings settings)
        {
            foreach (var processor in BuiltInLineProcessors)
            {
                if (processor is MarkdownTag markdownTag)
                {
                    if(markdownTag.GetMarkdownOpenTag(settings).Contains(s) || markdownTag.GetMarkdownCloseTag(settings).Contains(s))
                    {
                        return true;
                    }
                }
        ...
        public static bool IsOpenTag(this string s,MarkdownRenderingSettings settings,out string closingTag)
        {
            ...
            foreach (var processor in BuiltInLineProcessors)
            {
                if (processor is MarkdownTag markdownTag)
                {
                    if (s == markdownTag.GetMarkdownOpenTag(settings))
                    {
                        closingTag = markdownTag.GetMarkdownCloseTag(settings);
                        return true;
                    }
                }
            }
        ...
        public static bool IsCloseTag(this string s,MarkdownRenderingSettings settings,out string openTag)
        {
            ...
            foreach (var processor in BuiltInLineProcessors)
            {
                if (processor is MarkdownTag markdownTag)
                {
                    if (s == markdownTag.GetMarkdownCloseTag(settings))
                    {
                        openTag = markdownTag.GetMarkdownOpenTag(settings);
                        return true;
                    }
                }
            }
